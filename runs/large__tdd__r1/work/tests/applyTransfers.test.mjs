...
test('applyTransfers moves the debtor balance toward zero and the creditor down', () => {
...
test('applyTransfers does not mutate its input balances object', () => {
  const balances = { a: 66, b: -33, c: -33 };
...
test('applying settle(b) to b always yields an all-zero sheet', () => {
  const balances = { a: -10, b: 30, c: -40, d: 20 };
...
test('applyTransfers throws RangeError when to is not a key of balances', () => {
  assert.throws(
